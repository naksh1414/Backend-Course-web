...
app.use(
  cors({
    origin: ["https://sitelms.gogetskill.in", "https://www.youtube.com","http://localhost:8000","http://localhost:5173"],
    methods: ["POST", "GET"],
    credentials: true,
  })
);
...
app.get("/pdf-proxy/:courseId", async (req, res) => {
  ...
  const query = "SELECT file FROM pdf_notes WHERE course_id = ?";
  pool.query(query, [courseId], (err, result) => {
    if (err) {
      console.error("Error executing query:", err);
      res.status(500).json({ error: "Internal Server Error" });
